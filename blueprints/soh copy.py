...
def template():
    ...
    file_name = f"SOH_ALLBRANDS_{cust_part}_{today_str}.xlsx"
...
def _post_ledger_adjust(customer_id:int, sku_id:int, d:date, new_qty:float, old_qty:float,
                        hdr_id:int, row_num:int):
    diff = float(new_qty) - float(old_qty)
    # Zero-diff still ok to record for audit, but you can skip if you prefer
    idem = f"SOH_ADJ:{hdr_id}:{row_num}"
    model.add(SP_InventoryLedger(
        CustomerID=customer_id, SKU_ID=sku_id, DocDate=d,
        MovementType="ADJUST", MovementSubType=None,
        Qty=diff,
        UploadID=None,
        RefTable="SP_SOH_Uploads", RefID=str(hdr_id),
        IdempotencyKey=idem,
        CreatedAt=datetime.utcnow()
    ))
...
def upload():
    try:
        ...
        # ----- Block duplicate 'Initial' snapshot for this customer -----
        if snapshot_type.lower() == "initial":
            # Any prior non-superseded INITIAL header for the *same* customer should block this upload
            exists_initial = (model.query(SP_SOH_Uploads.SOHUploadID)
                              .filter(SP_SOH_Uploads.CustomerID == customer_id,
                                      func.lower(SP_SOH_Uploads.SnapshotType) == "initial")
                                      # treat anything not 'Superseded' as still existing
                                      # SP_SOH_Uploads.Status != "Superseded")
                              .first())
            if exists_initial:
                return jsonify(
                    ok=False,
                    error="Initial SOH already exists for this customer. "
                          "Please Select a 'Supersed' snapshot type instead."
                ), 400
        ...
        # --- Resolve/Create SKUs & build detail records ---
        records = []
        with _begin_tx(model):
            for _, r in df.iterrows():
                ...
                records.append({
                    ...
                    "SOHQty":     qty,
                    "RowNumber":  rn,
                    # Per-policy we snapshot a single date (the_date)
                })
            ...
            # Deactivate prior actives for this customer & date (no brand scoping)
            prior_hdr_q   = (model.query(SP_SOH_Uploads)
                             .filter(SP_SOH_Uploads.CustomerID==customer_id,
                                     SP_SOH_Uploads.Date==the_date))
            prior_headers = prior_hdr_q.all()
            prior_hdr_ids = [h.SOHUploadID for h in prior_headers]

            deactivated_total, inserted_total = 0, 0
            superseded_ids_total = []

            if prior_hdr_ids:
                deactivated = (model.query(SP_SOH_Detail)
                               .filter(SP_SOH_Detail.SOHUploadID.in_(prior_hdr_ids),
            ...
            # New header (no Brand now; it’s a mixed-brand file)
            hdr = SP_SOH_Uploads(
                CustomerID     = customer_id,
            ...
            detail_objs = []
            for rec in records:
                old_qty = _get_prior_active_qty_for_day(customer_id, None, rec["SKU_ID"], the_date)
                ...
                _post_ledger_adjust(
                    customer_id=customer_id, sku_id=rec["SKU_ID"], d=the_date,
                    new_qty=rec["SOHQty"], old_qty=old_qty,
                    hdr_id=hdr.SOHUploadID, row_num=rec["RowNumber"]
                )

            model.add_all(detail_objs)
            inserted_total += len(detail_objs)
            
            # ---- Mark customer Active if it has at least one ADJUST row ----
            # (This route just created ADJUST rows; still double-check for safety.)
            has_adjust = model.query(SP_InventoryLedger.CustomerID)\
            ...
            if has_adjust:
                ...
                model.query(SP_Customer)\
                     .filter(SP_Customer.CustomerID == customer_id,
                             (SP_Customer.StatusID != active_id) | (SP_Customer.StatusID.is_(None)))\
                     .update({
                         SP_Customer.StatusID: active_id,
                         # Prefer the snapshot date as status date; fall back to today if missing
